...
:: -----------------------------------------------------------
:: TELECHARGEMENT AUTOMATIQUE DES LOGICIELS
:: -----------------------------------------------------------
:choice
cls
echo La configuration est terminee.
echo.
echo Il reste une derniere etape.
echo.
set /p reponse=Tapez oui pour telecharger les logiciels de base, ou non pour quitter. (oui/non)
if /i "%reponse%"=="oui" (
    echo Telechargement et installation des logiciels de base...
    powershell -Command "Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/robin-portal/opti/main/ninite.exe' -OutFile '%TEMP%\ninite.exe'"
    start "" "%TEMP%\ninite.exe" /silent
) else if /i "%reponse%"=="non" (
    echo Installation annulee.
    pause
    exit /b
) else (
    echo Reponse invalide. Veuillez taper oui ou non.
    goto choice
)
